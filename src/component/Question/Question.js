...
const Question = ({questions}) => {
    ...
    return (
        <div className='question-container lg:mx-52 my-14'>
           ...
           <EyeIcon onClick={()=> seeAns(options)} className="h-8 w-8 ml-3  absolute top-0 right-2"/>
           </div>
          <div className='option md:grid md:grid-cols-2 gap-4'>
          {
            options.map(option => <p className='bg-blue-400 text-white rounded-3xl .lg:font-bold lg:text-xl mb-3 lg:mx-14 p-4' onClick={() => curectAns(option)}>{option}</p>)
           } 
           </div>           
        </div>
    );
